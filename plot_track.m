...
%         residual{wn} = zeros(N,3);
        ...
        for k = 1:N
            ...
            % stage one: get initial whale estimate by finding closest
            % point of intersection between two DOA lines (100 m
            % resolution):
...
%             w0 = mean([L1; L2]);
            ...
            %             fun = @(w)[s1.*sqrt(sum((w-h1).^2))+h1 - s2.*sqrt(sum((w-h2).^2)) - h2];
            
            %             [whaleLoc{wn}(k,:),  resnorm{wn}(k), residual{wn}(k,:)] = lsqnonlin(fun, w0, [], [], options);
            
            
            % Phase 2: 1 m resolution along DOA lines
                        
            r1mid = r1(n1);
            r2mid = r2(n2); % make Lsq minimum the center of the new search ranges
            
            r1 = (r1mid-100:1:r1mid+100)';
            r2 = (r2mid-100:1:r2mid+100)';
            ...
            r1best = r1(round(mean(n1)));
            r2best = r2(round(mean(n2)));
...
for wn = 1:8
    if ~isempty(REMORA.track{wn})
        ...
        scatter3(REMORA.track{wn}.whaleLoc(:,1), REMORA.track{wn}.whaleLoc(:,2), -REMORA.track{wn}.whaleLoc(:,3),  ms, colscale, 'filled')
        hold on
...
for wn = 1:8
    if ~isempty(REMORA.track{wn})
        if ~isempty(REMORA.track{wn}.t)
            ...
            scatter(REMORA.track{wn}.t, REMORA.track{wn}.whaleLoc(:,3), ms, col(wn,:), 'filled')
            hold off
            axis ij
            datetick
            grid on
